...
const Canvas = ({ ...props }) => {
  ...
  const templateData = {
    caption: {
      text:
        props.content || "1 & 2 BHK Luxury Apartments at just Rs.34.97 Lakhs",
      position: { x: 50, y: 100 },
      font_size: 44,
      alignment: "left",
      text_color: "#FFFFFF",
  ...
  useEffect(() => {
    ...
    maskImg.onload = () => {
      ...
      strokeImg.onload = () => {
        ctx.drawImage(strokeImg, 0, 0, ctx.canvas.width, ctx.canvas.height);
        ...
        drawTextWithWrap(
          ...
          templateData.caption.font_size + 10
        );
        ...
        ctx.beginPath();
        ctx.roundRect(
          templateData.cta.position.x - 99,
          templateData.cta.position.y - 55,
          templateData.cta.text.length + 200,
          templateData.caption.font_size + 48,
          10
        );
        ctx.fill();
